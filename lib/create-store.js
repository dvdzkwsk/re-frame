...
function createStore(initialState) {
  // Atom containing the current state of the store.
  //
  // Think of an "atom" as a box that holds a value. The box itself never
  // changes. Instead, we just replace its contents. We can then share the box,
  // allowing others to look inside of it whenever its contents change.
  //
  // This is how APP_DB works. When an event gets dispatched, we check to see
  // if it produced a change to "db" (this will be the case with almost every
  // EventDB handler). If "db" did change, then we use it to replace what's
  // currently in the atom.
  ...
  function processEvent(event) {
    ...
    if (!interceptors) {
      throw new Error(
        "You dispatched an event type that isn't registered with the store. " +
          'Please register "' +
          event[0] +
  ...
  var RUN_EFFECTS = {
    ...
    after: function after(context) {
      ...
      for (var effectId in effects) {
        ...
        if (__DEV__) {
          if (!handler) {
            var eventId = context.coeffects.event
            throw new Error(
              'The EventFX handler "' +
                eventId +
